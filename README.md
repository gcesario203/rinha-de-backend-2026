...
| **Índice** | **Ball tree** persistida em `references.balltree.bin`; busca em profundidade **closer-first** com poda por distância ao quadrado e fila de k vizinhos. |
...
1. **Ball tree em vez de força bruta**  
   Varredura linear em 3M × 14 floats por pedido não cabe no orçamento de latência. A ball tree reduz visitas com bounds geométricos; em 14D a poda não é “mágica”, mas ainda ganha muito face ao scan completo.
...
4. **Pré-build no Docker (`--prebuild`)**  
   Construir `references.bin` e a ball tree no **build da imagem** evita minutos de CPU/I-O no arranque do contentor, cumpre health checks curtos e remove dependência de volume partilhado lento entre réplicas.

5. **Cache em disco da árvore (formato próprio)**  
   Serialização pré-order com **centróide e raio também nas folhas** — necessário para a poda na busca ser equivalente à árvore em memória (versões antigas do cache sem isso degradavam recall drasticamente.
...
- `src/anti-fraud-core` — ball tree, dataset mmap, serialização JSON partilhada, motor k-NN / filas.
...
**.NET 10** minimal APIs, **k-NN (k = 5)** over a **memory-mapped** reference set (~3M × 14 floats), **ball tree** index baked at **Docker build** time (`references.balltree.bin`), **HAProxy** fronting two replicas, **source-generated JSON**, **SIMD** distance helpers, **warmup** before readiness, **JIT + TieredPGO + ReadyToRun** (Native AOT was tried and reverted for this numeric-heavy workload). See the Portuguese section above for the full rationale table.
